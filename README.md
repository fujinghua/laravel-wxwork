...
WxWork SDK for Laravel， 基于 [overtrue/wechat](https://github.com/fujinghua/wechat)
...
```shell
composer require "fujinghua/laravel-wxwork:~4.0"
```
...
'providers' => [
    // ...
    FuJinghua\LaravelWxWork\ServiceProvider::class,
],
'aliases' => [
    // ...
    'EasyWeChat' => FuJinghua\LaravelWxWork\Facade::class,
],
```
...
```shell
php artisan vendor:publish --provider="FuJinghua\LaravelWxWork\ServiceProvider"
```
...
```php
$app->register(FuJinghua\LaravelWxWork\ServiceProvider::class);
```

2. 如果你习惯使用 `config/wechat.php` 来配置的话，将 `vendor/fujinghua/laravel-wxwork/src/config.php` 拷贝到`项目根目录/config`目录下，并将文件名改成`wechat.php`。
...
protected $routeMiddleware = [
    // ...
    'wechat.oauth' => \FuJinghua\LaravelWxWork\Middleware\OAuthAuthenticate::class,
];
```
...
- OAuth 网页授权：`FuJinghua\LaravelWxWork\Events\WeChatUserAuthorized`
...
'open_platform' => [
    'uri' => 'serve',
    'action' => FuJinghua\LaravelWxWork\Controllers\OpenPlatformController::class,
    'attributes' => [
...
- 授权方成功授权：`FuJinghua\LaravelWxWork\Events\OpenPlatform\Authorized`
- 授权方更新授权：`FuJinghua\LaravelWxWork\Events\OpenPlatform\UpdateAuthorized`
- 授权方取消授权：`FuJinghua\LaravelWxWork\Events\OpenPlatform\Unauthorized`
- 开放平台推送 VerifyTicket：`FuJinghua\LaravelWxWork\Events\OpenPlatform\VerifyTicketRefreshed`
